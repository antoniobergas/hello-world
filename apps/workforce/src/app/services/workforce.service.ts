...
export class WorkforceService {
  ...
  private readonly _leaveRequests$ = new BehaviorSubject<LeaveRequest[]>([
    ...
    },
    {
      ...
      reviewedAt: new Date('2025-07-06'),
  ...
  approveLeave(id: string, reviewedBy: string): void {
    const requests = this._leaveRequests$
      .getValue()
      .map((r) =>
        r.id === id
          ? { ...r, status: 'approved' as LeaveStatus, reviewedAt: new Date(), reviewedBy }
          : r,
      );
  ...
  rejectLeave(id: string, reviewedBy: string): void {
    const requests = this._leaveRequests$
      ...
      .map((r) =>
        r.id === id
          ? { ...r, status: 'rejected' as LeaveStatus, reviewedAt: new Date(), reviewedBy }
          : r,
      );
    this._leaveRequests$.next(requests);
  }
