      ...
      @for (metric of filteredMetrics(); track metric.id) {
        <div class="metric-card">
          <div class="metric-header">
            ...
            <button class="refresh-btn" aria-label="Refresh metric" (click)="refresh(metric.id)">
              ↻
            </button>
          </div>
          <div class="metric-value">
            {{ metric.value | number: '1.0-2' }} <span class="metric-unit">{{ metric.unit }}</span>
          </div>
          ...
          <div
            class="metric-change"
            [ngClass]="{ positive: metric.changePercent > 0, negative: metric.changePercent < 0 }"
          >
            {{ metric.changePercent > 0 ? '+' : '' }}{{ metric.changePercent | number: '1.1-1' }}%
          </div>
          @if (metric.target !== undefined) {
            <div class="metric-target">
              Target: {{ metric.target | number: '1.0-2' }} {{ metric.unit }}
            </div>
          }
