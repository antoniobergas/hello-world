...
describe('AnnouncementService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({ providers: [AnnouncementService] });
    service = TestBed.inject(AnnouncementService);
  });

  it('should be created', () => { expect(service).toBeTruthy(); });
  it('should initialize with 8 seed announcements', () => { expect(service.getAll().length).toBe(8); });
  ...
  it('should create adds to announcements', () => {
  ...
  it('should getForAudience all includes all active all-audience announcements', () => {
    const forAll = service.getForAudience('all');
    expect(forAll.every(a => a.active)).toBe(true);
    expect(forAll.some(a => a.targetAudience === 'all')).toBe(true);
  });
  it('should getForAudience admins includes all-targeted and admin-targeted', () => {
    const forAdmins = service.getForAudience('admins');
    forAdmins.forEach(a => {
      expect(a.targetAudience === 'all' || a.targetAudience === 'admins').toBe(true);
    });
  });
  it('should getForAudience tenant returns tenant-specific and all', () => {
