...
const SEED_REPORTS: Report[] = [
  {
    ...
    entity: 'tickets',
    columns: [
      { field: 'id', label: 'ID', type: 'string', visible: true },
...
export class ReportBuilderService {
  ...
  create(
    data: Omit<Report, 'id' | 'createdAt' | 'runCount' | 'columns' | 'filters' | 'status'>,
  ): Report {
    const report: Report = {
      ...data,
  ...
  save(id: string): void {
    this.reportsSubject.next(
      this.reports.map((r) => (r.id === id ? { ...r, status: 'saved' } : r)),
    );
  }

  addColumn(id: string, column: ReportColumn): void {
    this.reportsSubject.next(
      this.reports.map((r) => (r.id === id ? { ...r, columns: [...r.columns, column] } : r)),
    );
  }
  ...
  removeColumn(id: string, field: string): void {
    this.reportsSubject.next(
      this.reports.map((r) =>
        r.id === id ? { ...r, columns: r.columns.filter((c) => c.field !== field) } : r,
      ),
    );
  }
  ...
  addFilter(id: string, filter: ReportFilter): void {
    this.reportsSubject.next(
      this.reports.map((r) => (r.id === id ? { ...r, filters: [...r.filters, filter] } : r)),
    );
  }
  ...
  removeFilter(id: string, field: string): void {
    this.reportsSubject.next(
      this.reports.map((r) =>
        r.id === id ? { ...r, filters: r.filters.filter((f) => f.field !== field) } : r,
      ),
    );
  }
  ...
  run(id: string): Report | undefined {
    ...
    this.reportsSubject.next(
      this.reports.map((r) =>
        r.id === id ? { ...r, runCount: r.runCount + 1, lastRunAt: new Date() } : r,
      ),
    );
  ...
  schedule(id: string, cronExpression: string): void {
    this.reportsSubject.next(
      this.reports.map((r) =>
        r.id === id ? { ...r, status: 'scheduled', schedule: cronExpression } : r,
      ),
    );
  }
