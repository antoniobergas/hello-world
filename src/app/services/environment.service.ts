...
const SEED_ENVIRONMENTS: Environment[] = [
  { id: 'env1', name: 'Dev T1', slug: 'dev-t1', type: 'development', tenantId: 't1', config: {}, status: 'active', deployCount: 5 },
  { id: 'env2', name: 'Staging T1', slug: 'staging-t1', type: 'staging', tenantId: 't1', config: {}, status: 'active', deployCount: 3 },
  { id: 'env3', name: 'Prod T1', slug: 'prod-t1', type: 'production', tenantId: 't1', config: {}, status: 'locked', lockedBy: 'u1', deployCount: 10 },
  { id: 'env4', name: 'Dev T1 B', slug: 'dev-t1-b', type: 'development', tenantId: 't1', config: {}, status: 'inactive', deployCount: 0 },
  { id: 'env5', name: 'Dev T2', slug: 'dev-t2', type: 'development', tenantId: 't2', config: {}, status: 'active', deployCount: 2 },
  { id: 'env6', name: 'Staging T2', slug: 'staging-t2', type: 'staging', tenantId: 't2', config: {}, status: 'active', deployCount: 1 },
  { id: 'env7', name: 'Prod T2', slug: 'prod-t2', type: 'production', tenantId: 't2', config: {}, status: 'active', deployCount: 7 },
  { id: 'env8', name: 'Dev T2 B', slug: 'dev-t2-b', type: 'development', tenantId: 't2', config: {}, status: 'locked', lockedBy: 'u2', deployCount: 0 },
];
...
export class EnvironmentService {
  ...
  unlock(id: string): void {
    this.environmentsSubject.next(
      this.environments.map(e => e.id === id ? { ...e, status: 'active', lockedBy: undefined } : e)
    );
  }
  ...
  approvePromotion(promotionId: string, approvedBy: string): void {
    this.promotionsSubject.next(
      this.promotions.map(p => p.id === promotionId ? { ...p, status: 'approved', approvedBy } : p)
    );
  }
  ...
  deployPromotion(promotionId: string): void {
    ...
    if (!promotion) return;
    this.promotionsSubject.next(
      this.promotions.map(p => p.id === promotionId ? { ...p, status: 'deployed', deployedAt: new Date() } : p)
    );
    this.environmentsSubject.next(
