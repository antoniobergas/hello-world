...
const SEED_COMMANDS: Command[] = [
  ...
  },
  {
    id: 'cmd2',
    label: 'Go to Tickets',
    description: 'Navigate to tickets list',
    icon: 'ticket',
    shortcut: 'g t',
    category: 'navigation',
    action: 'navigate:/tickets',
    keywords: ['issues', 'support'],
  },
  {
    id: 'cmd3',
  ...
  },
  {
    ...
    keywords: ['team', 'members', 'permissions'],
  },
  {
    id: 'cmd16',
    label: 'API Keys',
  ...
  },
  {
    ...
    keywords: ['changelog', 'updates', 'version'],
  },
  {
    id: 'cmd21',
...
export class CommandPaletteService {
  ...
  readonly recent$: Observable<Command[]> = this.recentSubject.pipe(
    map((ids) =>
      ids.map((id) => this.commandsSubject.getValue().find((c) => c.id === id)!).filter(Boolean),
    ),
  );
